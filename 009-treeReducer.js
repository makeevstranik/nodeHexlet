...
const treeReducer = (f, tree, acc) => { // принимаем три аргумента, эта ф. будет использована callback далее
  ...
  if (!children) {
    return acc; // конец рекурсии вернуть acc
  } // если есть ребенок - отправить его на новый цикл reduce
  return children.reduce((iAcc, n) => treeReducer(f, n, iAcc), acc); // здесь для каждого элемента n ребенка
  // применить ф. c аккумулятором iAcc - принимает acc. Эта функция в своем теле вызывает treeReducer
  // передавая туда f - прежднюю логику, текущий элемент ребенка, iAcc (acc).
};
